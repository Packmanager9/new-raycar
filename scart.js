...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class SpringOP {
        ...
        balance() {
            ...
            this.anchor.xmom = ((this.anchor.xmom*2 )+ xmomentumaverage) / 3
            this.anchor.ymom = ((this.anchor.ymom*2) + ymomentumaverage) / 3
        }
        draw() {
            this.beam.draw()
        }
    ...
    class Observer {
        constructor(x, y, radius, color, range = 100, rays = 10, angle = (Math.PI * .125)) {
            this.body = new Circle(x, y, radius, color, 0, 0 , .9)
            this.color = color
            this.ray = []
            this.rayrange = range
                    ...
                    if (this.ray[t].collided < 1) {
                        ...
                        for (let q = 0; q < this.obstacles.length; q++) {
                            if (this.obstacles[q].isPointInside(this.ray[t])) {
                                ...
                                this.xmom+=this.ray[t].xmom*.98
                                this.ymom+=this.ray[t].ymom*.98
                                this.anchored = 1
                            }
                        }
                    }
        ...
        draw() {
            ...
            for (let y = 0; y < this.ray.length; y++) {
                ...
                canvas_context.strokeStyle =  `rgb(${y*5}, ${255-(y*5)}, ${(y*y)*.1})`
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    let floor = new Rectangle(-5000, 600, 10000, 1000, "orange")
    let ramp = new Triangle(900, 600, "red", 300, 1)


    class Car{
        constructor(){
            this.body = new Observer(300, 300, 2, "blue", 30, 50, Math.PI*2)
            this.wheel = new Observer(350, 300, 2, "yellow", 50, 50, Math.PI*2)
            this.spring = new SpringOP(this.body.body, this.wheel.body, 100, 4, "white")
            this.obstacles = [floor, ramp]
        }
        draw(){
            this.spring.balance()
            this.spring.balance()
            ...
            this.spring.balance()
            this.spring.balance()
            this.body.obstacles = this.obstacles
            this.wheel.obstacles = this.obstacles
            if(this.body.anchored == 1){

                this.body.body.ymom*=.5
                let angle = this.spring.beam.angle()-(Math.PI*.5)+.2

                this.body.body.xmom += Math.cos(angle)*1.01
                this.body.body.ymom += Math.sin(angle)*1.01
            }else{
                this.body.body.ymom += .55
            }
            if(this.wheel.anchored == 1){
                
                this.wheel.body.ymom*=.5
            let angle = this.spring.beam.angle()-(Math.PI*.5)+.2

            this.wheel.body.xmom += Math.cos(angle)*1.01
            this.wheel.body.ymom += Math.sin(angle)*1.01
            }else{

            this.wheel.body.ymom += .55
            }
            this.body.body.xmom -= this.body.xmom*.7
            this.wheel.body.xmom -= this.wheel.xmom*.7
            this.body.body.ymom -= this.body.ymom*.7
            this.wheel.body.ymom -= this.wheel.ymom*.7
            this.body.body.frictiveMove()
            this.wheel.body.frictiveMove()
            this.body.draw()
            this.wheel.draw()
            this.spring.draw()
            ...
            let angle = this.spring.beam.angle()
            
            this.body.body.xmom += Math.cos(angle)*.5
            this.body.body.ymom += Math.sin(angle)*.5
            this.wheel.body.xmom += Math.cos(angle)*.5
            this.wheel.body.ymom += Math.sin(angle)*.5
                this.wheel.body.x -= 1
                this.body.body.x -= 1
                this.body.globalangle -= ((Math.PI*2)*.02)
                this.wheel.globalangle -= ((Math.PI*2)*.0167)
            }
            if(keysPressed['d']){
                let angle = this.spring.beam.angle()
                ...
                this.wheel.body.xmom -= Math.cos(angle)*.5
                this.wheel.body.ymom -= Math.sin(angle)*.5
                this.wheel.body.xmom -= .05
                this.body.body.xmom -= .05
                this.wheel.body.x += 1
                this.body.body.x += 1
                this.body.globalangle -= ((Math.PI*2)*.02)
                this.wheel.globalangle -= ((Math.PI*2)*.0167)
